...
@export_range(0.05, 0.5, 0.05) var inertia : float = 0.1
@export_range(0.05, 0.5, 0.05) var brake_inertia : float = 0.075
...
@onready var potion_effects : Node2D = $PotionEffects
...
func play_potion_effect(effect : String):
	var potion_effect : GPUParticles2D = find_free_particle_emitter()
	if potion_effect:
		potion_effect.process_material.color_ramp = load(effect)
		potion_effect.emitting = true	

func pick_up(object : Globals.Pickups) -> bool:
	return inventory.insert_item(object)

func find_free_particle_emitter() -> GPUParticles2D:
	for emitter in potion_effects.get_children():
...
func _on_area_2d_area_entered(area: Area2D) -> void:
	print("desi ghee") # Replace with function body.
